...
namespace LMS.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        string CreateNewUser( string firstName, string lastName, DateTime DOB, string departmentAbbrev, string role )
        {
            ...
            {
                int higestAdmin = db.Administrators.AsEnumerable().Max(s => int.Parse(s.UId.Substring(1)));
                higestID = Math.Max(higestID, higestAdmin);
            }
            if (db.Professors.Any())
            {
                int higestProf = db.Professors.AsEnumerable().Max(s => int.Parse(s.UId.Substring(1)));
                higestID = Math.Max(higestID, higestProf);
            }
            if (db.Students.Any())
            {
                int higestStudent = db.Students.AsEnumerable().Max(s => int.Parse(s.UId.Substring(1)));
                higestID = Math.Max(higestID, higestStudent);
            }


            string uID = "u" + (higestID + 1).ToString("D7"); // TODO

            // TODO: get dID using departmenAber

            uint dID = 0;
            if (role != "Administrator")
            {
                dID = db.Departments.Where(d => d.Subject == departmentAbbrev).Select(d => d.DId).FirstOrDefault();
            }

            // TODO: using that uID, create new user in database either in "Administrator", "Professor", or "Student"
            ...
            else if (role == "Professor")
            {
                db.Professors.Add(new Professor
                {
                    ...
                    Dob = DateOnly.FromDateTime(DOB),
                    DId = dID
                    // TODO: get Department ID from Abbr
                });
            }
